...
async function scrollPastOneViewport(page: import('@playwright/test').Page) {
	await page.evaluate(() => {
		window.scrollTo({
			top: Math.min(document.documentElement.scrollHeight, window.innerHeight * 1.5),
			behavior: 'auto',
		});
	});
}

test('Back to top reveals after one viewport and returns focus to main content', async ({
	page,
}) => {
...
test('Back to top is global on internal routes and keeps its Spanish accessible name', async ({
	page,
}) => {
	await page.setViewportSize(DESKTOP);
	...
	await expect(page.locator('#back-to-top-button')).toHaveAttribute(
		'aria-label',
		'Volver al inicio'
	);
});
