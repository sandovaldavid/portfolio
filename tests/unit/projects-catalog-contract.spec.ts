...
describe('Projects Catalog composition contract', () => {
	...
	it('anchors desktop catalog cards to opposite shell edges while preserving the wider Home composition', () => {
		...
		expect(projects).toContain('lg:grid-cols-[repeat(2,minmax(0,560px))]');
		...
		expect(projects).toContain('lg:gap-12');
		expect(projects).not.toContain('lg:gap-20');
	...
	it('gives catalog cards a distinct desktop composition and balanced footer actions', () => {
		...
		expect(card).toContain("isCatalog ? 'lg:max-w-148' : 'lg:max-w-140'");
